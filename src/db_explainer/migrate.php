...
if($checkC > 0){
  ...
  if(!in_array($update,$existing)){
  logger($user->data()->id,"Migrations","$update migration triggered for $plugin_name");
  $db->query("ALTER TABLE plg_db_explainer_columns ADD COLUMN db_id int(11) UNSIGNED NOT NULL AFTER id");
  $existing[] = $update; //add the update you just did to the existing update array
  $count++;
  }
...
$db->query("ALTER TABLE plg_db_explainer_databases ADD COLUMN diagram_is_public tinyint(1) NOT NULL DEFAULT '0' AFTER last_updated");
$db->query("ALTER TABLE plg_db_explainer_databases ADD COLUMN required_perms varchar(1000) default '2' AFTER diagram_is_public");
$db->query("ALTER TABLE plg_db_explainer_databases ADD COLUMN required_tags varchar(1000) default '' AFTER required_perms");
