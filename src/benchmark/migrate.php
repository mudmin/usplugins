...
if($checkC > 0){
  ...
  if(!in_array($update,$existing)){
  logger($user->data()->id,"Migrations","$update migration triggered for $plugin_name");
    $db->query("ALTER TABLE plg_benchmark ADD COLUMN randostring varchar(255)");

    $db->query("CREATE TABLE plg_benchmark_join (
      id int(11) UNSIGNED NOT NULL PRIMARY KEY AUTO_INCREMENT,
      benchdata varchar(255) NOT NULL,
      randostring varchar(255),
      total int(11),
      ts TIMESTAMP
    )");
  $existing[] = $update; //add the update you just did to the existing update array
