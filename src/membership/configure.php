...
if (!empty($_POST['plugin_membership'])) {
  ...
  if ($perms == '') {
    Redirect::to('admin.php?view=plugins_config&plugin=membership&err=You+must+add+a+permission+level&edit=' . $edit);
  }
          ...
          <div class="card-header">
            <div class="row">
              ...
              <div class="col-12 col-sm-3 text-end">
                <input type="submit" name="memset" value="Save" class="btn btn-outline-primary">
              </div>
            </div>
            ...
            <div class="row">
              ...
              <div class="col-5 form-group">
                ...
                <select class="" name="payments">
                  <option value="0" <?php if ($memset->payments == 0) {
                                      echo "selected='selected'";
                                    } ?>>No</option>
                  <option value="1" <?php if ($memset->payments == 1) {
                                      echo "selected='selected'";
                                    } ?>>Yes</option>
                </select>
              </div><br>
  ...
  <div class="row">
    <div class="col-sm-7 col-12">
      ...
      <div class="card">
        <div class="card-header">
          <h3><?php if ($e) {
                echo "<span style='background-color:yellow;'>Update</span>";
              } else {
                echo "Add New";
              } ?> Level</h3>
        </div>
        <div class="card-body">
          <form class="" action="<?php if ($e) {
                                    echo "admin.php?view=plugins_config&plugin=membership&edit=$edit";
                                  } ?>" method="post">
            <input type="hidden" name="csrf" value="<?= $token ?>" />

            <div class="form-group">
              <label for="plan_name">Level Name*</label>
              <input class="form-control" type="text" name="plan_name" value="<?php if ($e) {
                                                                                echo $thisPlan->plan_name;
                                                                              } ?>" required>
            </div>

            <div class="form-group">
              <label for="plan_desc">Description*</label>
              <input class="form-control" type="text" name="plan_desc" value="<?php if ($e) {
                                                                                echo $thisPlan->plan_desc;
                                                                              } ?>" required>
            </div>

            <div class="form-group">
              <label for="icon">Icon Filename (Just the filename located in usersc/membership/icons)</label>
              <input class="form-control" type="text" name="icon" value="<?php if ($e) {
                                                                            echo $thisPlan->icon;
                                                                          } ?>">
            </div>
            ...
            <div class="form-group">
              ...
              foreach ($perms as $p) { ?>
                <div class="col-4">
                  <input type="checkbox" name="perm[]" value="<?= $p->id ?>" <?php if ($e && in_array($p->id, $thesePerms)) {
                                                                              echo "checked";
                                                                            } ?>> <?= ucfirst($p->name) . " ($p->id)" ?>
                </div>
              <?php } ?>
            ...
            <div class="form-group">
              ...
              <input class="form-control" type="number" name="ordering" value="<?php if ($e) {
                                                                                  echo $thisPlan->ordering;
            ...
            <div class="form-group">
              ...
              <input class="form-control" type="text" name="script_add" value="" placeholder="i.e. begin_amateur_plan.php">
            </div>
            <div class="form-group">
              <label for="script">Optional - Script when plan is cancelled</label>
              <input class="form-control" type="text" name="script_remove" value="" placeholder="i.e. end_amateur_plan.php">
            </div>
            <div class="form-group">
              <?php if ($e) { ?>
        ...
        <div class="card-body">
          <form class="" action="<?php if ($e) {
            ...
            <div class="form-group">
              ...
              <input class="form-control" type="number" name="days" value="" min="1" step="1" placeholder="i.e. 30" required>
            </div>

            <div class="form-group">
              <label for="cost">Cost - No Symbols</label>
              <input class="form-control" type="number" name="cost" value="" min=".00" step=".01" placeholder="i.e. 30.00" required>
            </div>

            <div class="form-group">
              <label for="descrip">Description - We will automatically add the number of days to this description</label>
              <input class="form-control" type="text" name="descrip" value="" placeholder="i.e. 1 month" required>
            </div>
        ...
        <tbody>
          ...
          foreach ($levels as $l) { ?>
            <tr>
              ...
              <td>
                <a href="<?=$us_url_root?>users/admin.php?view=plugins_config&plugin=membership&edit=<?= $l->id ?>" class="btn btn-success">Edit</a>
              </td>
            </tr>
          <?php } ?>
        </tbody>
