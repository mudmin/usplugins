...
if($user->data()->plg_mem_expired == 0){
  if($user->data()->plg_mem_exp < date("Y-m-d H:i:s")){
    $fields = array(
      'plg_mem_expired'=>1,
    );
    $db->update('users',$user->data()->id,$fields);
    changeOfPlans($user->data()->plg_mem_level,0,$user->data()->id);
global $currentPage;
  if(isset($user) && $user->isLoggedIn() && $currentPage != 'login.php' && $currentPage != 'logout.php' &&  $currentPage != 'maintenance.php' &&  $currentPage != 'account.php' && !hasPerm([2],$user->data()->id)){
