				...
				<b>Exp. Date:</b> <?php if ($u->plg_mem_exp < $date) {
					...
					<?php
							if (!empty($_GET['submitFinal'])) {
								...
								if (!empty($selected->script)) {
									// Sanitize the filename to prevent directory traversal (e.g., ../../)
									$safeScript = basename($selected->script);
									$scriptPath = $abs_us_root . $us_url_root . 'usersc/plugins/membership/scripts/' . $safeScript;

									// Verify the file exists and is a regular file before including
									if (file_exists($scriptPath) && is_file($scriptPath)) {
										include $scriptPath;
									} else {
										// Optional: log that the script was missing
										logger($user->data()->id, "Membership Error", "Script not found: " . $safeScript);
									}
								}

								// Call the changeOfPlans function (Hardened in your functions.php earlier)
								changeOfPlans((int)$u->plg_mem_level, (int)$selected->id, (int)$u->id);
								Redirect::to('manage_members.php?err=' . urlencode($whatHappened));
							} ?>
				<?php } ?>
				<?php if (!$ps && !$sel) { ?>
					<?php if ($e) { ?>
