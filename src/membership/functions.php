...
function changeOfPlans($from,$to,$uid){
  ...
  if($from > 0){
    ...
    if($f->script_remove != '' && file_exists($abs_us_root.$us_url_root.'usersc/plugins/membership/scripts/'.$f->script_remove)){
      include $abs_us_root.$us_url_root.'usersc/plugins/membership/scripts/'.$f->script_remove;
    }
  }
  if($to > 0){
    ...
    if($f->script_add != '' && file_exists($abs_us_root.$us_url_root.'usersc/plugins/membership/scripts/'.$f->script_add)){
      include($abs_us_root.$us_url_root.'usersc/plugins/membership/scripts/'.$f->script_add);
    }
  }
}
