...
if($checkC > 0){
  ...
  $update = '00002';
  if(!in_array($update,$existing)){
    $db->query("ALTER TABLE stripe_transactions CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;");
    // $db->query("ALTER TABLE stripe_transactions ADD COLUMN live tinyint(1)");
    // $db->query("ALTER TABLE stripe_transactions ADD COLUMN trans_type VARCHAR(255)");
    $existing[] = $update; //add the update you just did to the existing update array
  $count++;
  }
