...
?>
  <div class="form-group">
        ...
        value="<?php
                if ($u == 1) {
                  ...
                  if (isset($_POST[$o->col])) {
                    echo htmlspecialchars($_POST[$o->col], ENT_QUOTES, 'UTF-8');
                  }
                } ?>"
        <?php if ($o->required == 1) {
          echo "required";
    ...
    ?>
      <input type="number" step="1" <?php if ($o->field_type == "tinyint") {
                                      echo "min='0' max='9'";
                                    } ?> name='<?= $o->col ?>' id='<?= $o->col ?>' class='<?= $o->field_class ?>'
        value="<?php if ($u == 1) {
                  echo htmlspecialchars($value, ENT_QUOTES, 'UTF-8');
                } elseif (!empty($_POST)) {
                  echo htmlspecialchars($_POST[$o->col], ENT_QUOTES, 'UTF-8');
    ...
    ?>
      <input type="time" name='<?= $o->col ?>' pattern="^([0-1]?[0-9]|2[0-4]):([0-5][0-9])(:[0-5][0-9])?$" id='<?= $o->col ?>' placeholder="23:59 / 23:59:59" class='<?= $o->field_class ?>'
        ...
        <?= html_entity_decode($o->input_html) ?>><?php
      ...
      <textarea name='<?= $o->col ?>' id='<?= $o->col ?>' class='<?= $o->field_class ?>'
        <?php if ($o->required == 1) {
                                                  echo "required";
                                                } ?> <?= html_entity_decode($o->input_html) ?>><?php if ($u == 1) {
                                                                                                  echo htmlspecialchars($value, ENT_QUOTES, 'UTF-8');
                                                                                                } elseif (!empty($_POST)) {
                                                                                                  echo htmlspecialchars($_POST[$o->col], ENT_QUOTES, 'UTF-8');
                                                                                                } ?></textarea>
    ...
    if ($o->field_type == "date") { ?>
      <input type="date" class="form-control" name="<?= $o->col ?>" id="<?= $o->col ?>" value="<?php if ($u == 1) {
                                                                                              echo htmlspecialchars($value, ENT_QUOTES, 'UTF-8');
                                                                                            } elseif (!empty($_POST)) {
                                                                                              echo htmlspecialchars($_POST[$o->col], ENT_QUOTES, 'UTF-8');
                                                                                            } ?>">
    ...
    if ($o->field_type == "datetime") { ?>
      <input type="datetime-local" class="form-control" name="<?= $o->col ?>" id="<?= $o->col ?>"
        value="<?php if ($u == 1) {
                  echo htmlspecialchars(date("Y-m-d\TH:i:s", strtotime($value)), ENT_QUOTES, 'UTF-8');
                } elseif (!empty($_POST)) {
                  echo
                  htmlspecialchars(date("Y-m-d\TH:i:s", strtotime($_POST[$o->col])), ENT_QUOTES, 'UTF-8');
                } ?>">
      <?php
    }
    ...
    if ($o->field_type == "radio") {
      ...
      ?>
        <div class="radio">
          <label><input type="radio" value="<?= $k ?>" <?php if ($u == 1) {
                                                        if ($value == $k) {
                                                          echo "checked='checked'";
                                                        }
                                                      } ?> <?php echo $o->input_html; ?> name='<?= $o->col ?>'>
            <?php
            if (str_starts_with($v, '(LANG)')) {
              echo lang(substr($v, 6));
            } else {
...
function displayForm($name, $opts = [])
{
  ...
  // Validate form name to prevent path traversal and SQL injection
  $name = basename($name);
...
?>
  ...
  function displayView($view, $opts = [])
  {
    ...
    // Validate form_name from database to prevent path traversal and SQL injection
    $getView->form_name = basename($getView->form_name);
    if (!preg_match('/^[a-zA-Z0-9_]+$/', $getView->form_name)) {
      die("Invalid form name");
    }
  ...
  function preProcessForm($opts = [])
  {
    ...
    // Validate table name contains only safe characters (alphanumeric and underscores)
    // and use basename to prevent path traversal in file includes
    $name = basename($name);
    if (!preg_match('/^[a-zA-Z0-9_]+$/', $name)) {
      $response['errors'] = "Invalid form name";
      return $response;
    }
    $form = $name . '_form';
    $fields = [];
    ...
    $hookDir1 = $abs_us_root . $us_url_root . "usersc/plugins/forms/hooks/before_form_process/";
    $hookFile1 = $hookDir1 . $name . ".php";
    $realHookFile1 = realpath($hookFile1);
    if ($realHookFile1 && strpos($realHookFile1, realpath($hookDir1)) === 0 && file_exists($realHookFile1)) {
      include $realHookFile1;
    }
    $errors = [];
    $successes = [];
    $errorArray = [];
    ...
    if (!$errorArray == []) {
      $hookDir2 = $abs_us_root . $us_url_root . "usersc/plugins/forms/hooks/form_validation_fail/";
      $hookFile2 = $hookDir2 . $name . ".php";
      $realHookFile2 = realpath($hookFile2);
      if ($realHookFile2 && strpos($realHookFile2, realpath($hookDir2)) === 0 && file_exists($realHookFile2)) {
        include $realHookFile2;
      }
      ...
      $hookDir3 = $abs_us_root . $us_url_root . "usersc/plugins/forms/hooks/form_validation_success/";
      $hookFile3 = $hookDir3 . $name . ".php";
      $realHookFile3 = realpath($hookFile3);
      if ($realHookFile3 && strpos($realHookFile3, realpath($hookDir3)) === 0 && file_exists($realHookFile3)) {
        include $realHookFile3;
      }
    }
  ...
  function postProcessForm($response, $opts = [])
  {
    ...
    // Validate response name to prevent path traversal and SQL injection
    $response['name'] = basename($response['name']);
    if (!preg_match('/^[a-zA-Z0-9_]+$/', $response['name'])) {
      $response['errors'] = "Invalid form name";
      return $response;
    }

    // Verify table name exists in us_forms whitelist before allowing update/insert
    $tableCheck = $db->query("SELECT id FROM us_forms WHERE form = ?", [$response['name']]);
    if ($tableCheck->count() < 1) {
      $response['errors'] = "Invalid form table";
      return $response;
    }

    if (isset($usFormUpdate)) {
      // Validate that usFormUpdate is a positive integer to prevent SQL injection
      if (!is_numeric($usFormUpdate) || (int)$usFormUpdate != $usFormUpdate || $usFormUpdate <= 0) {
        $response['errors'] = "Invalid update ID";
        return $response;
      }
      $usFormUpdate = (int)$usFormUpdate;
    ...
    $hookDir = $abs_us_root . $us_url_root . "usersc/plugins/forms/hooks/after_form_process/";
    $hookFile = $hookDir . $response['name'] . ".php";
    $realHookFile = realpath($hookFile);
    if ($realHookFile && strpos($realHookFile, realpath($hookDir)) === 0 && file_exists($realHookFile)) {
      include $realHookFile;
    }
  ...
  function duplicateForm($new, $old)
  {
    ...
    if (!preg_match('/^[a-zA-Z0-9_]+$/', $new) || !preg_match('/^[a-zA-Z0-9_]+$/', $old)) {
      usError("Invalid form names provided.");
      Redirect::to($us_url_root . 'users/admin.php?view=plugins_config&plugin=forms&newFormView=_admin_forms');
      exit;
    }
    ...
    if ($check['success'] == true) {
      ...
      // These queries are now safe because of the preg_match check above
      $query = $db->query("CREATE TABLE `$new` LIKE `$old` ");
      $new_form_table = $new . "_form";
      $old_form_table = $old . "_form";

      $query = $db->query("CREATE TABLE `$new_form_table` LIKE `$old_form_table` ");
      $copy = $db->query("SELECT * FROM `$old_form_table` ")->results(true);
      foreach ($copy as $c) {
        $db->insert($new_form_table, $c);
      }
