				...
				if ($length != 0) {
					switch ($rule) {
						...
						if (is_numeric($value)) {
							...
							if ($rule=="lessthan"  &&  $value>$rule_value){
								$str = lang("VAL_LESS");
								$this->addError(["{$display} $str {$rule_value_display}",$item]);
							}

							if ($rule=="greaterthan"  &&  $value<$rule_value){
								$str = lang("VAL_LESS");
								$this->addError(["{$display} $str {$rule_value_display}",$item]);
							}

							if ($rule=="greaterthanequal"  &&  $value>=$rule_value){
								$str = lang("VAL_LESS_EQ");
								$this->addError(["{$display} $str {$rule_value_display}",$item]);
							}

							if ($rule=="lessthanequal"  &&  $value<=$rule_value){
								$str = lang("VAL_GREAT_EQ");
								$this->addError(["{$display} $str {$rule_value_display}",$item]);
							}

							if ($rule=="notequal"  &&  $value!=$rule_value){
								$str = lang("VAL_NOT_EQ");
								$this->addError(["{$display} $str {$rule_value_display}",$item]);
							}

							if ($rule=="equal"  &&  $value==$rule_value){
								$str = lang("VAL_EQ");
								$this->addError(["{$display} $str {$rule_value_display}",$item]);
							}
						}
