...
$orgC = $orgQ->count();
if ($orgC < 1 || $org < 2) {
  Redirect::to($us_url_root . 'users/admin.php?view=plugins_config&plugin=saas&v=org&err=Org+not+found');
}
$org = $orgQ->first();
...
if (!empty($_POST['createOrg'])) {
  ...
  if ($org->owner != Input::get('owner')) {
    ...
    if ($check > 0) {
      ...
      Redirect::to($us_url_root . 'users/admin.php?view=plugins_config&plugin=saas&v=manage_org&o=' . $o->id . '&err=Owner+could+not_be+changed');
    }
  ...
  <div class="col-12 col-md-6">
    <div class="card">
      ...
      <div class="card-body">
        ...
        <b>Note:</b> Deactivating an org, deactivates all of its users EXCEPT the owner. The owner will still be able to login,
