				...
				<form class="" action="manage_inventory.php?edit=<?=$edit?>" method="post">
					...
					<?php }else{?>
 					...
 					<?php }
						foreach($cats as $c){
							?>
						<option <?php if($last == $c->id){echo "selected";} ?> value="<?=$c->id?>"><?=echoCat($c->id);?></option>
						<?php } ?>
					</select>

					<label for="">Item Name*</label>
					<input type="text" name="item" value="<?php if($edit != ''){ echo $item->item;}?>" class="form-control" required>

					<label for="">Long Item Description</label>
					<textarea name="Description" rows="3" cols="80" class="form-control"><?php if($edit != ''){ echo $item->description;}?></textarea>

					<label for="">Price*</label>
					<input type="number" name="price" value="<?php if($edit != ''){ echo $item->price;}?>" min="0.00" step="0.01" class="form-control" required>

					<label for="">Quantity on Hand (QOH)*</label>
					<input type="number" name="qoh" value="<?php if($edit != ''){ echo $item->qoh;}?>" min="0" step="1" class="form-control" required>

					<!-- <label for="">Cost of Goods</label>
					<input type="number" name="cost" value="<?php //if($edit != ''){ echo $item->cost;}?>" min="0.00" step="0.01" class="form-control"> -->
					<label for="">Is this a digital/non-physical item</label>
					<select class="form-control" name="digital" required>
						<option value="0" <?php if(($edit != '') && ($item->digital == 0)){ echo "selected";}?>>No</option>
						<option value="1" <?php if(($edit != '') && ($item->digital == 1)){ echo "selected";}?>>Yes</option>
					</select>

					<label for="">Disabled</label>
					<select class="form-control" name="disabled" required>
						<option value="0" <?php if(($edit != '') && ($item->disabled == 0)){ echo "selected";}?>>No</option>
						<option value="1" <?php if(($edit != '') && ($item->disabled == 1)){ echo "selected";}?>>Yes</option>
					</select>
			...
			<div class="col-12">
				...
				<?php if($edit > 0){
					...
					<table class="table table-striped">
						<thead>
							<tr>
								<th>Photo (Click for full)</th><th>Filename</th><th>Delete</th>
							</tr>
						</thead>
						<tbody>
