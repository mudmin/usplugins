		...
		<div class="row">
			<div class="col-sm-6">
				...
				<form class="" action="categories.php?edit=<?=$edit?>" method="post">
					...
					<input type="text" name="cat" class="form-control" value="<?php if($edit != ''){ echo $item->cat;}?>">
			...
			<form class="" action="categories.php?edits=<?=$edits?>" method="post">
				...
				<select class="form-control" name="cat">
					...
					<?php foreach($cats as $c){?>
						<option <?php if(($edits != '') && ($sub->subcat_of == $c->id)){ echo "selected";}?> value="<?=$c->id?>"><?=$c->cat?></option>
					<?php } ?>
				</select>
				<label for="">Sub Category*</label>
				<input type="text" name="subcat" class="form-control" value="<?php if($edits != ''){ echo $sub->cat;}?>">

				<label for="">Sub Category Disabled?</label>
				<select class="form-control" name="disabled" required>
					<option value="0" <?php if(($edits != '') && ($sub->disabled == 0)){ echo "selected";}?>>No</option>
					<option value="1" <?php if(($edits != '') && ($sub->disabled == 1)){ echo "selected";}?>>Yes</option>
				</select>
				<!-- <label for="">Photo*</label>
				<input type="text" name="photo" value="" class="form-control"> -->
				<br>
		...
		<?php if($edit != ''){ ?>
			<div class="row">
				<div class="col-6 offset 3">
					...
					<h3>Add Photo to the Selected <?php if($edits != ''){echo "Sub-";}?>Category</h3>
					<?php if (!empty($_FILES)) {
